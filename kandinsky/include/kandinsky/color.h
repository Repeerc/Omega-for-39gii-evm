...
public:
  ...
  static constexpr KDColor RGB8(uint8_t rgb) {
    return KDColor(rgb);
  }
  static constexpr KDColor RGB16(uint16_t rgb) {
	  return KDColor(((((rgb&0xF800)>>11) * 8)+(((rgb&0x07E0)>>5) * 4)+(((rgb&0x001F)) * 8))/3);
  }
  static constexpr KDColor RGB24(uint32_t rgb) {
	  
	return KDColor( ((((rgb&0xFF0000)>>16))+(((rgb&0x00FF00)>>8))+(((rgb&0x0000FF))))/3);
  }
  static constexpr KDColor RGB888(uint8_t r, uint8_t g, uint8_t b) {
	  return KDColor((r+g+b)/3);
  }
  uint8_t red() const {
    return m_value;
  }
  ...
  //operator uint16_t() const { return (uint16_t)m_value; }
  operator uint16_t() const { return m_value; }
private:
  constexpr KDColor(uint8_t value) : m_value(value) {}
  uint8_t m_value;
};
